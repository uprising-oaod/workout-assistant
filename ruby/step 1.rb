...
p ["массивы гетерогенны", 42, [4, "тест"]] # создание массива путем перечисления элементов
p Array.new(6){ |index| index + 1 }    #=> [1, 2, 3, 4, 5, 6] создание массива, через вызов метода .new класса Array
p (1..6).to_a    #=> [1, 2, 3, 4, 5, 6] Ещё один способ создания массива - создании объекта типа Range (диапазон) и вызове метода .to_a
# Эти три способа самые распространенные
...
p array = ("a".."e").to_a    #=>  ["a", "b", "c", "d"]
# отрицательная индексация.
p array[array.size - 2]                #=> "d" Аналогично нижеследующему
p array[-2]                            #=> "d" Такая запись короче
...
p ["а", "б", "вг", "д", "в"].max_by{ |elem| elem.size }    #=> "вг" максимальный по размеру строки
# аналогично min и min_by

# Упорядочение аналогично вызывается метод sort или sort_by
# массивы упорядочиваются по возрастанию
# Пример сортировки для двумерного массива
p [[1,0], [16,6], [2,1], [4,5],[4,0],[5,6]].sort_by {|elem| elem[1]}
 #=> [[1, 0], [4, 0], [2, 1], [4, 5], [16, 6], [5, 6]] сортировка "внешних" элементов по значению "внутренних"
p [[1,0], [16,6], [2,1], [4,5],[4,0],[5,6]].sort_by {|elem| elem[0]}
